function []=tmult_testcases(testCases,debugin)
% This file contains lots of test-cases to test the performance of the tmult
...
% ans=tmult(randn(100000,1),1,randn(100000,1)',1); 
% ans=tmult(randn(100000,1),1,randn(100000,1)',1);
...
unitTest([str ' OuterProduct, [1],[2]'],tmult(X(:,1),1,Y(:,1),2),X(:,1)*Y(:,1).');
unitTest([str ' Inner product, [-1],[-1]'],tmult(X(:,1),-1,Y(:,1),-1),X(:,1).'*Y(:,1));
unitTest([str ' Matrix product, [1 -1],[-1 2]'],tmult(X(:,:,1),[1 -1],Y,[-1 2]),X(:,:,1)*Y);
unitTest([str ' transposed matrix product, [-1 1],[-1 2]'],tmult(X(:,:,1),[-1 1],Y,[-1 2]),X(:,:,1).'*Y);
unitTest([str ' Matrix frobenius norm, [-1 -2],[-1 -2]'],tmult(X(:,:,1),[-1 -2],Y,[-1 -2]),sum(sum(X(:,:,1).*Y)));

unitTest([str ' transposed matrix frobenius norm, [-1 -2],[-2 -1]'],tmult(X(:,:,1),[-1 -2],Y,[-2 -1]),sum(sum(X(:,:,1).'.*Y)));

unitTest([str ' ignored dims, [0 -2],[-2 2 1]'],tmult(Y(1,:),[0 -2],X(:,:,:),[-2 2 1]),reshape(Y(1,:)*reshape(X,size(X,1),[]),size(X,2),size(X,3)).');


% Higher order matrix operations
unitTest([str ' spatio-temporal filter [-1 -2 1],[-1 -2]'],tmult(X,[-1 -2 1],Y,[-1 -2]),reshape(Y(:).'*reshape(X,size(X,1)*size(X,2),size(X,3)),[size(X,3) 1]));

unitTest([str ' spatio-temporal filter (fallback) [-1 -2 1],[-1 -2]'],tmult(X,[-1 -2 1],Y,[-1 -2]),reshape(Y(:).'*reshape(X,size(X,1)*size(X,2),size(X,3)),[size(X,3) 1]));

unitTest([str ' spatio-temporal filter (order) [-1 -2],[-1 -2 1]'],tmult(Y,[-1 -2],X,[-1 -2 1]),reshape(Y(:).'*reshape(X,size(X,1)*size(X,2),size(X,3)),[size(X,3) 1]));

unitTest([str ' spatio-temporal filter (order) [-1 -2],[-1 -2 3]'],tmult(Y,[-1 -2],X,[-1 -2 3]),reshape(Y(:).'*reshape(X,size(X,1)*size(X,2),size(X,3)),[size(X,3),1]));

unitTest([str ' transposed spatio-temporal filter [1 -2 -3],[-2 -3]'],tmult(X,[1 -2 -3],Y,[-2 -3]),reshape(reshape(X,size(X,1),size(X,2)*size(X,3))*Y(:),[size(X,1) 1]));

unitTest([str ' matrix-vector product [-1 1 2][-1]'],tmult(X,[-1 1 2],Y(:,1),[-1]),reshape(Y(:,1).'*reshape(X,[size(X,1) size(X,2)*size(X,3)]),[size(X,2) size(X,3)]));

unitTest([str ' spatial filter (fallback): [-1 2 3],[-1 1]'],tmult(X,[-1 2 3],Y,[-1 1]),reshape(Y.'*reshape(X,[size(X,1) size(X,2)*size(X,3)]),[size(Y,2) size(X,2) size(X,3)]));

unitTest([str ' spatial filter: [-1 2 3],[-1 1]'],tmult(X,[-1 2 3],Y,[-1 1]),reshape(Y.'*reshape(X,[size(X,1) size(X,2)*size(X,3)]),[size(Y,2) size(X,2) size(X,3)]));

unitTest([str ' temporal filter [1 -2 3],[2 -2]'],tmult(X,[1 -2 3],Y(1,:),[2 -2]),squeeze(sum(X.*repmat(Y(1,:),[size(X,1) 1 size(X,3)]),2)));

unitTest([str ' temporal filter [2 -2],[1 -2 3]'],tmult(Y(1,:),[2 -2],X,[1 -2 3]),squeeze(sum(X.*repmat(Y(1,:),[size(X,1) 1 size(X,3)]),2)));

unitTest([str ' temporal filter [-2 2],[1 -2 3]'],tmult(Y(1,:).',[-2 2],X,[1 -2 3]),sum(X.*repmat(Y(1,:),[size(X,1) 1 size(X,3)]),2));

unitTest([str ' temporal filter [-2 2],[1 -2 3]'],tmult(Y(1,:).',[-2 2],X,[1 -2 3]),sum(X.*repmat(Y(1,:),[size(X,1) 1 size(X,3)]),2));

Xp=permute(X,[1 3 2]);
unitTest([str ' blk-code [-1 1 -2][-1 2 -2]'],tmult(X,[-1 1 -2],X,[-1 2 -2]),reshape(Xp,[],size(Xp,3)).'*reshape(Xp,[],size(Xp,3)));
...
tic; for i=1:1000; Z=tmult(X(:,1),1,Y(:,1),2); end
fprintf('%30s %gs\n','tmult',toc/1000); % = .05  / .01
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic; for i=1:1000; Zm=tmult(X(:,1),1,Y(:,1),2); end
fprintf('%30s %gs\n','tmult m',toc/1000); % = .05  / .01
A=complex(randn(size(X)),randn(size(X))); % flush cache?
...
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic, for i=1:1000;Z=tmult(X(:,:,1),[1 -1],Y,[-1 2]);end
fprintf('%30s %gs\n','tmult',toc/1000);% = .28 / .06
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic, for i=1:1000;Zm=tmult(X(:,:,1),[1 -1],Y,[-1 2]);end
fprintf('%30s %gs\n','tmult m',toc/1000);% = .28 / .06
A=complex(randn(size(X)),randn(size(X))); % flush cache?
...
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic, for i=1:1000;Z=tmult(X(:,:,1),[-1 1],Y,[2 -1]);end
fprintf('%30s %gs\n','tmult',toc/1000);% =.3 / .06 
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic, for i=1:1000;Zm=tmult(X(:,:,1),[-1 1],Y,[2 -1]);end
fprintf('%30s %gs\n','tmult m',toc/1000);% =.3 / .06 
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic, for i=1:1000;T=X(:,:,1).'*Y.';end
fprintf('%30s %gs\n','MATLAB',toc/1000); % =.17  / .06
...
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic,for i=1:500;Z=tmult(X,[-1 -2 1],Y,[-1 -2]);end,
fprintf('%30s %gs\n','tmult',toc/500);% =.26 / .18
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic,for i=1:500;Zm=tmult(X,[-1 -2 1],Y,[-1 -2]);end,
fprintf('%30s %gs\n','tmult m',toc/500);% =.26 / .18
A=complex(randn(size(X)),randn(size(X))); % flush cache?
...
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic,for i=1:50;Z=tmult(X,[1 -2 -3],Y,[-2 -3]);end,
fprintf('%30s %gs\n','tmult',toc/50);% =.27 / .28
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic,for i=1:50;Zm=tmult(X,[1 -2 -3],Y,[-2 -3]);end,
fprintf('%30s %gs\n','tmult m',toc/50);% =.27 / .28
A=complex(randn(size(X)),randn(size(X))); % flush cache?
...
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic,for i=1:500; Z=tmult(X,[-1 1 2],Y(:,1),[-1]);end,
fprintf('%30s %gs\n','tmult',toc/500); %=.27 / .26
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic,for i=1:500; Zm=tmult(X,[-1 1 2],Y(:,1),[-1]);end,
fprintf('%30s %gs\n','tmult m',toc/500); %=.27 / .26
A=complex(randn(size(X)),randn(size(X))); % flush cache?
...
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic;for i=1:500;Z=tmult(X,[-1 2 3],Y(:,1:2),[-1 1]);end;
fprintf('%30s %gs\n','tmult',toc/500);%=.39/.37
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic;for i=1:500;Zm=tmult(X,[-1 2 3],Y(:,1:2),[-1 1]);end;
fprintf('%30s %gs\n','tmult m',toc/500);%=.39/.37
A=complex(randn(size(X)),randn(size(X))); % flush cache?
...
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic,for i=1:50; Z=tmult(X,[1 -2 3],Y,[2 -2]);end
fprintf('%30s %gs\n','tmult',toc/50); %=.27 / .31
A=complex(randn(size(X)),randn(size(X))); % flush cache?
...
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic;for i=1:50;Z=tmult(X,[1 -1 3],[],[2 -1 3]);end;
fprintf('%30s %gs\n','tmult',toc/50);  %=8.36/7.6
A=complex(randn(size(X)),randn(size(X))); % flush cache?
...
A=complex(randn(size(X)),randn(size(X))); % flush cache?
tic;for i=1:10;Z=tmult(X,[1 -1 2],[],[1 -1 3]);end;
fprintf('%30s %gs\n','tmult',toc/10); %=    /37.2
A=complex(randn(size(X)),randn(size(X))); % flush cache?
...
fprintf('[1 -1][-1]\n');
tic, for i=1:1000;Z=tmult(X,[1 -1],Y,[-1],'n');end
fprintf('%30s %gs\n','tmult',toc/1000);% = .28 / .06
tic, for i=1:1000;Z=tmult(X,[1 -1],Y,[-1],'mn');end
fprintf('%30s %gs\n','tmult m',toc/1000);% = .28 / .06
end
...
fprintf('[1 -1 -2][-1 -2]\n');
tic, for i=1:1000;Z=tmult(X,[1 -1 -2],Y,[-1 -2]);end
fprintf('%30s %gs\n','tmult',toc/1000);% = .28 / .06
tic, for i=1:1000;Z=tmult(X,[1 -1 -2],Y,[-1 -2]);end
fprintf('%30s %gs\n','tmult m',toc/1000);% = .28 / .06
fprintf('[-1 -2 1][-1 -2]\n');
tic, for i=1:1000;Z=tmult(X,[-1 -2 1],Y,[-1 -2]);end
fprintf('%30s %gs\n','tmult',toc/1000);% = .28 / .06
tic, for i=1:1000;Z=tmult(X,[-1 -2 1],Y,[-1 -2]);end
fprintf('%30s %gs\n','tmult m',toc/1000);% = .28 / .06
fprintf('[-1 2 3][-1 1]\n');
tic, for i=1:100;Z=tmult(X,[-1 2 3],Y,[-1 1]);end
fprintf('%30s %gs\n','tmult',toc/100);% = .28 / .06
tic, for i=1:100;Z=tmult(X,[-1 2 3],Y,[-1 1]);end
fprintf('%30s %gs\n','tmult m',toc/100);% = .28 / .06
end
...
for N=Ns;%[128,256,512,1024,2048];
  ...
  for i=1:5;
     ...
     % use tmult without matlab
     for b=1:numel(blkSzs); blkSz=blkSzs(b);
      clear T;T=randn(1024,1024); % clear cache
      tic,for j=1:3;tmult(X,[1 -1],Y,[-1 2],'mn',blkSz);end;
		tptime(b)=tptime(b)+toc;
    end;

    % tmult with defaults & matlab if possible
    clear T;T=randn(1024,1024); % clear cache
    tic,for j=1:3;tmult(X,[1 -1],Y,[-1 2]);end;
