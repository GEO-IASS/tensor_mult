function []=tmult_testcases_orig(testCases,debugin)
...
unitTest([str ' spatio-temporal filter (order) [-1 -2],[-1 -2 3]'],tmult(Y,[-1 -2],X,[-1 -2 3]),reshape(Y(:).'*reshape(X,size(X,1)*size(X,2),size(X,3)),[1 1 size(X,3)]));
...
unitTest([str ' temporal filter [1 -2 3],[2 -2]'],tmult(X,[1 -2 3],Y(1,:),[2 -2]),sum(X.*repmat(Y(1,:),[size(X,1) 1 size(X,3)]),2));
